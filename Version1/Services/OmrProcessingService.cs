...
namespace Version1.Services
{
    public class OmrProcessingService
    {
        ...
        public async Task<OmrResult> ProcessOmrSheet(string imagePath, string templatePath, string imageUrl)
        {
            ...
            var MatImgOut2 = await AlignWithTemplate(matInput, template2, "scan");
            ...
            Point2f[] scaningDetaction = new Point2f[]
           {
            ...
            foreach (var field in template["fields"])
            {
                ...
                if (bubblesArray != null)
                {
                    ...
                    //  OPEN - here we will add drawing to design bubble cordination on image and share to isBubble fill methord.
                    ...
                    foreach (var pt in bubbleRects)
                    {
                        ...
                        int diameter = Math.Min(pt.Width, pt.Height); // Take the smaller of width and height
                        ...
                        Cv2.Rectangle(matInput, topLeftBub, bottomRightBub, new Scalar(0, 0, 255), 1);

                        //// Crop the region of interest (ROI) inside the bounding box
                        //Mat roi = new Mat(matInput, new OpenCvSharp.Rect(x, y, width, height));
                        ...
                        //Cv2.DrawContours(matInput, contoursValConvty, -1, new Scalar(225, 255, 0), 1);

                        Cv2.Circle(matInput, (int)pt.X, (int)pt.Y, 1, new Scalar(0, 0, 255), -1);
                        //Cv2.Ellipse(matInput, new OpenCvSharp.Point(x, y), new OpenCvSharp.Size(width / 2, height / 2), 0, 0, 360, new Scalar(0, 0, 255), -1);
                        //Cv2.Circle(matInput, new OpenCvSharp.Point(x, y), diameter / 2, new Scalar(0, 0, 255), -1);
